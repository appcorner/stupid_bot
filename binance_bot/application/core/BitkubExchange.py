import numpy as np
import pandas as pd
from bitkub_async import BitkubAsync
...
class Bitkub:
    ...
    async def start_connection(self):
        self.client = BitkubAsync(self.api_key, self.api_secret)
        # self.client.set_api_key(self.api_key)
        # self.client.set_api_secret(self.api_secret)
        # self.bm = None # binance.BinanceSocketManager(self.client, user_timeout = 120)
    ...
    async def request_historical_candle_single(self,symbol):
        sv_tm = await self.client.servertime()
        dt_to = int(sv_tm)
        dt_gap = int(datetime.datetime.now().timestamp()) - dt_to
        dt_frm = int(dateparser.parse(self.parameters_on_data.historical_candle_lookback).timestamp()) + dt_gap
        # print(dt_to, dt_frm)
        val = await self.client.tradingview(sym=symbol, int=TF_MAP[self.parameters_on_data.candle_timeframe], frm=dt_frm, to=dt_to)
        # print(val)
        for res_pack in zip(val['t'], val['o'], val['h'], val['l'], val['c'], val['v']):
            candle = BitkubUtilities.request_historical_candle(symbol, res_pack)
    ...
    async def request_precision(self):
        res = await self.client.ticker()
        Event.log.notify(f'request precision (bk:ticker)')
        self.precision_data = BitkubUtilities.request_precision(res, self.watch_list)
...
class BitkubUtilities:
    ...
    def request_historical_candle(symbol, res):
        time = datetime.datetime.fromtimestamp(res[0])
    ...
    def request_precision(res, watch_list):
        ...
        for x in res.keys():
            if res[x] in watch_list:
                symbol = res[x]
                price_precision = res[x]['last']
                quantity_precision = res[x]['baseVolume']
    ...
    def order_order_endpoint(res, order_history, order_name):
        ...
        from_order = BitkubUtilities.match_order_name(order_history, 'symbol', symbol, 'order_name') if BitkubUtilities.is_exit_order(trade_direction,position_side) else ''
    ...
    def order_user_data_socket(res, order_history):
        ...
        if order_event != 'NEW':
            order_name = BitkubUtilities.match_order_name(order_history, 'order_id', order_id, 'order_name')
            from_order = BitkubUtilities.match_order_name(order_history, 'symbol', symbol, 'from_order') if BitkubUtilities.is_exit_order(trade_direction,position_side) else ''
            order_history.loc[-1] = [time, symbol, order_event, order_type, trade_direction, position_side, price, quantity, order_id, order_name, from_order]
            order_history.index += 1
        return order_history
    ...
    def portfolio_update_user_data_socket(res, portfolio_df):
        ...
        symbol = df['s'][0]
        portfolio_df.loc[symbol] = BitkubUtilities.portfolio_row_creator(symbol, df, 'pa','ep')
        return portfolio_df
